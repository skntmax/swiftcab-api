       ...
       loginByAuth : async function (req:Request, res:Response):Promise<any> {
        ...
        verifyMailLink : async  function (req:Request, res:Response):Promise<any> {
         ...
         try {
            ...
            const roleQuery = paramString(req.query.role)
            const roleNum = Number(roleQuery)
            let userVarify =await authService.verifyMailLink({userId : Number(userId) , username  ,role: roleNum, roleTypeName  })
